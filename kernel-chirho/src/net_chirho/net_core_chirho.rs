...
pub fn sys_recvfrom_chirho(
    ...
    // Child-gate: if this process (PID 3+) has a running child AND
    // the child hasn't written to its pipe yet, yield repeatedly to
    // let the child finish. This prevents PID 3 from reading SSH data
    // (which may contain CHANNEL_CLOSE) before the child writes output.
    // Without this, slow commands (sqlite3) get their CHANNEL_CLOSE
    // processed before their output reaches the pipe.
    {
        let recv_pid_chirho = crate::task_chirho::current_task_chirho()
            .map(|t| t.lock().pid_chirho).unwrap_or(0);
        if recv_pid_chirho >= 3 {
            // Gate when ANY descendant (child, grandchild, etc.) is running.
            // sqlite3 is PID 5 (grandchild: PID 3 → PID 4(sh) → PID 5(sqlite3)).
            // PID 4 exits quickly but PID 5 takes longer for dynamic linking.
            let has_descendant_chirho = {
                let list_chirho = crate::task_chirho::TASK_LIST_CHIRHO.lock();
                list_chirho.iter().any(|t_chirho| {
                    let tg_chirho = t_chirho.lock();
                    tg_chirho.pid_chirho > recv_pid_chirho
                        && !tg_chirho.is_exited_chirho()
                })
            };
            crate::serial_println_chirho!(
                "[CHILD-GATE-CHECK] pid={} has_desc={}",
                recv_pid_chirho, has_descendant_chirho,
            );
            if has_descendant_chirho {
                crate::serial_println_chirho!(
                    "[CHILD-GATE] pid={} has running descendant — gating recvfrom",
                    recv_pid_chirho,
                );
                // Yield until ALL descendants exit or pipe gets data
                for _yield_i_chirho in 0..10000u32 {
                    let child_done_chirho = {
                        let list_chirho = crate::task_chirho::TASK_LIST_CHIRHO.lock();
                        !list_chirho.iter().any(|t_chirho| {
                            let tg_chirho = t_chirho.lock();
                            tg_chirho.pid_chirho > recv_pid_chirho
                    ...
                    if child_done_chirho {
                        break;
                    }
                    // Check if pipe has data already
                    let pipe_has_data_chirho = (5u64..16).any(|pfd| {
                        if let Some(pf) = crate::fs_chirho::lookup_fd_chirho(pfd) {
                            let pg = pf.lock();
                            if (pg.inode_chirho.lock().mode_chirho & 0o170000) == 0o10000 {
                                if let Some(ref pd) = pg.inode_chirho.lock().fs_data_chirho {
                                    if let Some(pp) = pd.downcast_ref::<alloc::sync::Arc<spin::Mutex<crate::pipe_chirho::PipeChirho>>>() {
                                        return !pp.lock().buffer_chirho.is_empty();
                                    }
                                }
                            }
                        }
