...
fn pipe_common_chirho(fds_ptr_chirho: u64, flags_chirho: u32) -> i64 {
    ...
    if read_fd_chirho >= 9 {
        let (verify_chirho, table_ptr_chirho) = crate::task_chirho::current_task_chirho()
            .map(|t| {
                let tg = t.lock();
                if let Some(ref fdt) = tg.fd_table_chirho {
                    (fdt.get_chirho(read_fd_chirho as usize).is_some(),
                     fdt.fds_chirho.as_ptr() as u64)
                } else {
                    (false, 0)
                }
            }).unwrap_or((false, 0));
        let pid_chirho = crate::task_chirho::current_task_chirho()
            .map(|t| t.lock().pid_chirho).unwrap_or(0);
        crate::serial_println_chirho!(
            "[PIPE-VERIFY] pid={} fd={} in_table={} table_ptr={:#x}",
            pid_chirho, read_fd_chirho, verify_chirho, table_ptr_chirho,
        );
    }
