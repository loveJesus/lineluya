...
pub fn copy_from_user_chirho(
    ...
    // SAFETY: We have validated that `user_src_chirho` through
    // `user_src_chirho + len_chirho - 1` falls within user space.  The
    // actual physical mapping is assumed to be present (page-fault fixup is
    // not yet implemented).  `kernel_dst_chirho` is a valid mutable slice
    // with at least `len_chirho` bytes.
    unsafe {
...
pub fn copy_to_user_chirho(
    ...
    // SAFETY: Address range has been validated.  The kernel source slice is
    // guaranteed valid by Rust's type system.  Physical mapping presence is
    // assumed (see `copy_from_user_chirho` for the same caveat).
    unsafe {
        core::ptr::copy_nonoverlapping(
